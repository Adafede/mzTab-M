...
:document-version: 2.1.0-InDevelopment July 2025 (DEVELOPMENT)
...
mzTab-M 2.1.0 clarifies several aspects, and introduces the `study_variable_group` element to support multi-factorial experimental designs. As a consequence, the `study_variable[1-n]-factors` field has been removed: the experimental factors it was intended to capture are now represented explicitly through `study_variable_group[1-n]` and its sub-fields (`-description`, `-type`, `-datatype`, `-unit`), together with the `study_variable_group[1-n]-study_variable_refs` field by which each group lists the study variables that belong to it. The `-datatype` sub-field supports the standard XSD primitive types as well as `Parameter`, allowing study variable values to be reported as user-defined or CV Parameters when categorical factors are best expressed by ontology terms.
mzTab-M 2.1.0 also introduces _profiles_ (see <<profiles>>): an optional `mzTab-profile` metadata field that declares which of the four tables a file contains. This allows intermediate analysis outputs — for example feature-only files from preprocessing, or files carrying identification evidence but no aggregated summary — to be valid mzTab-M. Files that do not declare a profile remain valid: their profile is inferred from the tables present, preserving backwards compatibility with mzTab-M 2.0.0.
...
[#figurerrr-1]
image::img/media/figure1.svg[image, title="Simple experimental designs in mzTab-M can be represented using a combination of the elements study_variable (SV), sample, assay, ms_run and study_variable_group. Quantitative values can be reported in files for SVs and assays. A) SV is intended to capture different groups of replicates, which might have resulted from different levels of a given variable e.g. control versus treated (represented as 2 SVs), n time points over a treatment course (as n SVs). B) samples are optional in mzTab since the quantitative software may often be unaware of the biological samples that have been analyzed. If that information is available, references from assay to the same (technical, upper half) or different (biological, bottom half) samples are used to describe the type of replication performed. C) assay captures a measurement made about a molecule (small molecule/lipid) where multiple assays within the same group are taken to be replicates of some kind (biological or technical). D) ms_run captures a single run on an MS instrument. E) study_variable_group describe grouping of multiple SVs in multifactorial experimental designs (see <<reporting-factorial-experimental-designs,Section 5.3>>).",width=100%,pdfwidth=100%,scaledwidth=100%]

[[reporting-factorial-experimental-designs]]
=== Recommendations for reporting multifactorial experimental designs

Some explanation is needed about the reporting of multifactorial experimental designs, which are common in metabolomics. In mzTab-M 2.1, the `study_variable_group` element has been introduced to support the grouping of multiple study variables (SVs) that together represent a column in a wide-layout experimental design matrix. For example, if there are two factors (e.g. treatment and time point), each factor is one `study_variable_group`. Each study_variable is one level of a single factor and references the relevant assay(s); each `study_variable_group` in turn lists its member study variables via `study_variable_group[1-n]-study_variable_refs`. At least one `study_variable_group` MUST be defined.

[[figure-2]]
image::img/media/figure2.svg[image, title="The `study_variable_group` allows to include different types of multiple SVs, supporting multifactoral experimental designs. For instance, if the experiment includes SVs that can either describe the treatment type or the time point as shown in the figure, the `study_variable_group` helps distinguishing the groups while keeping the desired structure.",width=100%,pdfwidth=100%,scaledwidth=100%]
...
Evidence for small molecule identification is captured by reference from the SML table via features (SMFs) down to the final table - Small Molecule Evidence (SME) elements. It is possible to have a legal mzTab-M file that does not contain any features (a summary-level file, profile `M+S`); equally, a file MAY contain features and evidence but no summary (profiles `M+F` and `M+F+E`). Which cross-references are expected therefore depends on the file's profile (see <<profiles>>). It is generally RECOMMENDED, where the full evidence trail is available, to report data at the SML, SMF and SME levels (profile `M+S+F+E`).
...
The cross-reference obligations under each profile are summarised below.

[[table-profile-crossrefs]]
.Cross-reference rules per profile. `SMF_ID_REFS` links each SML row to its features; `SME_ID_REFS` links each SMF row to its evidence.
[cols="1,2,2",options="header",]
|===============================================================================
|Profile |SML -> SMF (`SMF_ID_REFS`) |SMF -> SME (`SME_ID_REFS`)
|`M` |n/a (no SML table) |n/a (no SMF table)
|`M+S` |MUST be null for every SML row (no SMF table to reference) |n/a (no SMF table)
|`M+F` |n/a (no SML table) |MUST be null for every SMF row (no SME table to reference)
|`M+F+E` |n/a (no SML table) |MUST contain at least one valid SME_ID for every SMF row that has been identified; MAY be null for unidentified features
|`M+S+F+E` |MUST contain at least one valid SMF_ID per SML row |as `M+F+E`
|===============================================================================

The `SME_ID_REF_ambiguity_code` on an SMF row MUST be null whenever that row's `SME_ID_REFS` is null — that is, in every profile without an SME section (`M`, `M+S`, `M+F`). It is used only to disambiguate SMF rows that reference more than one SME element.

[[figure-3]]
image::img/media/figure3.png[image,width=100%,pdfwidth=100%,scaledwidth=100%, title=" A) The summary level (SML) reports the final assumed identification, allowing for ambiguity by “|” separated results in the relevant columns. B) The feature level (SMF) does not explicitly report identifications but references down to the SME level. Ambiguity is propagated via referencing multiple SME elements (rows) with different identification results. C) One SME element (one row) represents a single possible identification from some input evidence. Multiple identifications from the same input data share the same value for evidence_input_id. Ambiguity is captured by different rows for the same input data."]
...
An mzTab-M file MUST contain a Metadata (MTD) section. The presence of the Small Molecule (SML), Small Molecule Feature (SMF) and Small Molecule Evidence (SME) sections is governed by the file's _profile_, declared in the `mzTab-profile` metadata field (see <<mztab-profile>>). A profile declares which tables are present in the file, and thereby which validation rule set applies. Five profiles are defined; the definitive list, and the tables each profile requires, is given in <<table-profiles>>.

Sections that ARE present MUST follow the table order MTD -> SML -> SMF -> SME, with a blank line separating each. Sections that are NOT required by the declared profile MUST be omitted entirely — no header line and no rows. An empty table (header line only, no rows) is NOT a substitute for omitting the section.

If the `mzTab-profile` field is absent — for example in mzTab-M 2.0 files, or files written before profiles were introduced — validators MUST infer the profile from the tables present and report the inferred profile in the validation log. New files SHOULD set `mzTab-profile` explicitly.

[[profile-metadata-requirements]]
==== Metadata fields whose requirement depends on the profile

Some metadata (MTD) fields are only meaningful when a particular table is present. Each such field is therefore MANDATORY in the profiles that include the corresponding table, and OPTIONAL — tolerated but not required — in the others. The rule is stated once per field, in terms of the table it depends on:

[cols="2,3",options="header",]
|===============================================================================
|MTD field |Required when…
|`quantification_method` |the file reports quantified molecules — an SML or SMF section is present (`M+S`, `M+F`, `M+F+E`, `M+S+F+E`)
|`small_molecule-quantification_unit` |the SML section is present (`M+S`, `M+S+F+E`)
|`small_molecule_feature-quantification_unit` |the SMF section is present (`M+F`, `M+F+E`, `M+S+F+E`)
|`id_confidence_measure[1-n]` |the SME section is present (`M+F+E`, `M+S+F+E`)
|`database[1-n]` |identification has been performed — an SME section is present, or SML rows carry identifiers
|===============================================================================

In profiles that do not include the relevant table these fields are OPTIONAL. If such a field is nevertheless present in a file whose profile does not include the corresponding table, a validator SHOULD accept the file but emit an informational message noting the profile/content mismatch (for example, a `small_molecule-quantification_unit` in an `M+F` file, which has no SML section for it to describe).

[[profiles-not-supported]]
==== Profiles that are explicitly not supported

Two table combinations are prohibited because they would contain cross-references that cannot be resolved:

* *M+E* (MTD + SME, evidence without features). SME rows are addressable only through `SME_ID_REFS` on SMF rows; an evidence-only file would therefore contain unreachable evidence. There is no realistic workflow that produces SME without SMF: even direct-infusion identification produces a feature-equivalent observation (a single mass at a single retention "time", with `retention_time_in_seconds` null), which mzTab-M models as an SMF row.
* *M+S+E* (MTD + SML + SME, no SMF). SML can reference SMF rows via `SMF_ID_REFS`, but SME is not directly addressable from SML. Allowing this combination would create evidence with no validated cross-reference path from the summary.

The combination *M+S+F* (MTD + SML + SMF, no SME) is permitted but discouraged. It corresponds to the summary-with-features pattern tolerated in earlier versions. Producers writing summary-only output SHOULD instead declare profile `M+S` and omit the SMF section; producers with feature-level data SHOULD use `M+S+F+E` with at least one SME row per identified SMF. `M+S+F` is retained only for backwards compatibility and is NOT RECOMMENDED for new production.

[[profile-lattice]]
==== Profile inclusion lattice

Each profile is a strict subset of the ones above it: any file valid under a given profile satisfies the structural expectations of the profiles it contains. Validators MAY be configured to accept a minimum richness (e.g. "`M+F` or richer") by checking the file against the profile that minimally covers its declared content.

....
                  M+S+F+E     (strict superset of all others)
                 /       \
              M+S+F      M+F+E
             /     \       |
          M+S      M+F -----+
...
include::mzTab_m_2_1_schema.adoc[]

[[changes-mztabm-2-0-to-2-1]]
== Changes from mzTab-M version 2.0 to 2.1

include::mzTab_m_2_0_to_2_1_changes.adoc[leveloffset=+1]
